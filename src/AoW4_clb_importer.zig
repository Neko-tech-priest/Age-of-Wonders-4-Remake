...
//     print("{x}\n", .{@intFromPtr(BlockTable.dataAfterHeaderPtr) - @intFromPtr(fileBuffer)});
    ...
    memcpy(mesh.*.name, stringsOffsetPtr+NameOffset, NameLen);
...
//     texture.*.name = stringsOffsetPtr+NameOffset;
    ...
    memcpy(texture.*.name, stringsOffsetPtr+NameOffset, NameLen);
...
//                     {
//                         texture_size = (tex_width*tex_height)/2;
//                         texture.*.format = VulkanInclude.VK_FORMAT_BC4_UNORM_BLOCK;
//                     },
//                     0x0b =>
//                     {
//                         texture_size = (tex_width*tex_height);
...
//                     },
                ...
                {
                    memcpy(texture.*.texture.data, dataBlockPtr+mem.bytesToValue(u32, mipmapTable.dataAfterHeaderPtr + 17), dataSize);
//                     break;
                }
...
//     print("{x}\n", .{@intFromPtr(mainMaterialChunk.dataAfterHeaderPtr) - @intFromPtr(fileBuffer)});
    ...
    switch(mainMaterialChunk.tablesCount)
    {
        113 =>
        {
            ...
            memcpy(material.DiffuseTexture, stringsOffsetPtr + mem.bytesToValue(u32, mainMaterialChunk.dataAfterHeaderPtr + mainMaterialChunk.header[13][1]+26+8), textureNameSize);
//             material.DiffuseTexture = stringsOffsetPtr + mem.bytesToValue(u32, mainMaterialChunk.dataAfterHeaderPtr + mainMaterialChunk.header[13][1]+26+8);
        ...
        43 =>
        {
            ...
            material.DiffuseTexture = (globalState.arenaAllocator.alignedAlloc(u8, customMem.alingment, textureNameSize) catch unreachable).ptr;
            material.DiffuseTextureLen = @intCast(textureNameSize);
            memcpy(material.DiffuseTexture, stringsOffsetPtr + mem.bytesToValue(u32, mainMaterialChunk.dataAfterHeaderPtr + mainMaterialChunk.header[2][1]+26+8), textureNameSize);
            //             material.DiffuseTexture = stringsOffsetPtr + mem.bytesToValue(u32, mainMaterialChunk.dataAfterHeaderPtr + mainMaterialChunk.header[13][1]+26+8);
            print("{s}\n", .{material.DiffuseTexture[0..textureNameSize]});
        },
        else =>
...
inline fn readChunk_Model(fileBuffer: [*]u8, fileBufferPtrIteratorIn: [*]u8, stringsOffsetPtr: [*]u8,) void
{
    _ = fileBuffer;
//     _ = fileBufferPtrIteratorIn;
...
pub fn clb_read(path: [*:0]const u8, meshesPtr: *[*]Mesh_temp, meshesCountPtr: *u32, texturesPtr: *[*]Texture_temp, texturesCountPtr: *u32, materialsPtr: *[*]Material_temp, materialsCountPtr: *u32) !void
{
...
//     print("dataOffset: {x}\n", .{@intFromPtr(dataOffsetPtr)-@intFromPtr(fileBuffer)});
    ...
    {
        // skip 0x0101
//         readTable(clb_Table.dataAfterHeaderPtr + clb_Table.header[2][1] + 3, &clb_Table.tables[2]);
//         const currentTable: *Table = &clb_Table.tables[0];
    }
...
//     var texturesCount: u64 = 0;
    ...
    while(tableIndex < headersTable.tablesCount) : (tableIndex+=1)
    {
...
//     meshes = (globalState.arenaAllocator.alloc(AoW4.Mesh, meshesCount) catch unreachable).ptr;
    materials = (globalState.arenaAllocator.alloc(Material_temp, materialsCount) catch unreachable).ptr;
//     materials = @ptrCast(@alignCast((c.malloc(materialsCount*@sizeOf(Material)))));
    materialsCount = 0;
    ...
    while(tableIndex < headersTable.tablesCount) : (tableIndex+=1)
    {
        ...
        if(chunkType == 0x0041004b)
        {
            print("{x}\n", .{@intFromPtr(fileBufferPtrIterator)-@intFromPtr(fileBuffer)});
            fileBufferPtrIterator+=4;
            readChunk_Model(fileBuffer, fileBufferPtrIterator,  stringsOffsetPtr);
            modelsCount+=1;
            print("\n", .{});
            //             break;
        }
...
//     print("totalCompressedTexturesSize: {x}\n", .{totalCompressedTexturesSize});
}
