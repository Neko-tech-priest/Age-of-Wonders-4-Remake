...
pub fn createVkBuffer(size: u64, usage:VulkanInclude.VkBufferUsageFlags, buffer: *VulkanInclude.VkBuffer) void
{
...
pub fn createVkBuffers__VkDeviceMemory_AoS(usage: VulkanInclude.VkBufferUsageFlags, srcStructArray: [*]u8, srcStructSize: u32, sizeOffset: u32, VkBufferStructArray: [*]u8, VkBufferStructSize: u32, numBuffers: u32, dstDeviceMemory: *VulkanInclude.VkDeviceMemory) void
{
	var sizeDeviceMemory: usize = 0;
	const buffers_full_sizes: [*]u64 = (globalState.arenaAllocator.alloc(u64, numBuffers) catch unreachable).ptr;
	var i: usize = 0;
	while(i < numBuffers) : (i+=1)
	{
		createVkBuffer(mem.bytesToValue(u32, srcStructArray+srcStructSize*i+sizeOffset), usage, @as(*VulkanInclude.VkBuffer, @ptrCast(@alignCast(VkBufferStructArray+VkBufferStructSize*i))));
		var memRequirements: VulkanInclude.VkMemoryRequirements = undefined;
		VulkanInclude.vkGetBufferMemoryRequirements(VulkanGlobalState._device, @as(*VulkanInclude.VkBuffer, @ptrCast(@alignCast(VkBufferStructArray+VkBufferStructSize*i))).*, &memRequirements);
	...
	while(i < numBuffers) : (i+=1)
	{
		...
		VK_CHECK(VulkanInclude.vkBindBufferMemory(VulkanGlobalState._device,  @as(*VulkanInclude.VkBuffer, @ptrCast(@alignCast(VkBufferStructArray+VkBufferStructSize*i))).*, dstDeviceMemory.*, deviceOffset));
		memcpy((@as([*]u8, @ptrCast(data))+deviceOffset), (@as(*PtrStruct, @ptrCast(@alignCast(srcStructArray+srcStructSize*i)))).*.data, @as(*u32,@ptrCast(@alignCast(srcStructArray+srcStructSize*i+sizeOffset))).*);
