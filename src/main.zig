...
pub fn main() !void
{
// 	print("suggestVectorLength: {?}\n", .{std.simd.suggestVectorLength(u64)});
// 	const vector = @Vector(1, u8);
// 	print("VectorCount: {t}\n", .{std.simd.VectorCount(@Vector(1, u8))});
...
//
	...
	var AoW4_meshes_temp: [*]AoW4_clb_importer.Mesh_temp = undefined;
	var AoW4_meshes: [*]AoW4.Mesh = undefined;
	var AoW4_meshesCount: u32 = 0;
	...
	var AoW4_images_temp: [*]AoW4_clb_importer.Texture_temp = undefined;
	var AoW4_images: [*]AoW4.DiffuseMaterial = undefined;
	var AoW4_imagesCount: u32 = 0;
...
// 	AoW4_meshIndex = 0;
	...
	try AoW4_clb_importer.clb_read("Age of Wonders 4/Content/Title/Libraries/Strategic/Spawner_Large_Monster_Strategic.clb", &AoW4_meshes_temp, &AoW4_meshesCount, &AoW4_images_temp, &AoW4_imagesCount, &AoW4_materials_temp, &AoW4_materialsCount);

	while(AoW4_materialIndex < AoW4_materialsCount) : (AoW4_materialIndex+=1)
	{
		while(AoW4_imageIndex < AoW4_imagesCount) : (AoW4_imageIndex+=1)
		{
			const textureLen: u64 = AoW4_materials_temp[@intCast(AoW4_materialIndex)].DiffuseTextureLen;
			if(mem.eql(u8, AoW4_materials_temp[@intCast(AoW4_materialIndex)].DiffuseTexture[0..textureLen], AoW4_images_temp[@intCast(AoW4_imageIndex)].name[0..textureLen]))
			{
				AoW4_materials_temp[@intCast(AoW4_materialIndex)].texture = &AoW4_images[@intCast(AoW4_imageIndex)];
				break;
			}
		}
		AoW4_imageIndex = 0;
		while(AoW4_meshIndex < AoW4_meshesCount) : (AoW4_meshIndex+=1)
		{

// 			const meshNameLen: u64 = AoW4_materials_temp[AoW4_materialIndex].DiffuseTextureLen;
// 			if(mem.eql(u8, AoW4_materials_temp[AoW4_materialIndex].DiffuseTexture[0..meshNameLen], AoW4_images_temp[@intCast(AoW4_imageIndex)].name[0..meshNameLen]))
// 			{
// 				AoW4_materials_temp[AoW4_materialIndex].texture = &AoW4_images[AoW4_imageIndex];
// 				break;
// 			}
		}
		AoW4_meshIndex = 0;
	}
	AoW4_materialIndex = 0;
// 	while(AoW4_imageIndex < AoW4_imagesCount) : (AoW4_imageIndex+=1)
...
// 	std.process.exit(0);
	...
	VkImage.createVkImages__VkImageViews__VkDeviceMemory_AoS(@as([*]u8, @ptrCast(@alignCast(AoW4_images_temp)))+0, @sizeOf(AoW4_clb_importer.Texture_temp), @ptrCast(@alignCast(AoW4_images)), @sizeOf(AoW4.DiffuseMaterial), AoW4_imagesCount, &AoW4_images_VkDeviceMemory);
	...
	AoW4.Create_DiffuseMaterial_VkDescriptorPool(&AoW4_imagesDescriptorPool, AoW4_imagesCount);
